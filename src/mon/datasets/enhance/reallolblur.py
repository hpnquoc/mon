...
class RealLOLBlur(VisionDataset):
    """Loads Real-LOL-Blur dataset from ``root`` dir.
    ...
    def __init__(self, root: pathlib.Path, *args, **kwargs):
        ...
        root = root / "reallolblur" if root.name != "reallolblur" else root
        if not root.is_dir():
...
# ----- DataModule -----
@DATAMODULES.register(name="reallolblur")
class RealLOLBlurDataModule(types.DataModule):
    ...
    def setup(self, stage: Literal["train", "test", "predict", None] = None):
        ...
        if stage in [None, "train"]:
            self.train = RealLOLBlur(split=Split.TEST, **self.dataset_kwargs)
            self.val   = RealLOLBlur(split=Split.TEST, **self.dataset_kwargs)
