...
"""Implements the paper: "You Only Learn One Representation: Unified Network for
Multiple Tasks,".
...
import i_test as test  # import test.py to get mAP after each epoch
import mon
from models.yolo import Model
from utils.datasets import create_dataloader
from utils.general import (
    check_dataset, check_file, check_git_status, check_img_size, fitness, fitness_ap,
    fitness_ap50, fitness_f1, fitness_p, fitness_r, get_latest_run, init_seeds, labels_to_class_weights,
...
# ----- Train -----
def run_train(hyp: dict, opt: dict, device: torch.device, tb_writer=None, wandb=None):
    logger.info(f"Hyperparameters {hyp}")
    root             = mon.Path(opt["root"])
    ...
    best_f1      = save_dir / "best_f1.pt"
    best_ap50    = save_dir / "best_ap50.pt"
    best_ap      = save_dir / "best_ap.pt"
    ...
    with open(opt["data"], encoding="utf-8") as f:
        ...
        if isinstance(train_, list):
            train_ = [str(root / "data" / t) for t in train_]
        elif train_:
            train_ = str(root / "data" / train_)
        if isinstance(val_, list):
            val_   = [str(root / "data" / t) for t in val_]
        elif val_:
            val_   = str(root / "data" / val_)
        if isinstance(test_, list):
            test_  = [str(root / "data" / t) for t in test_]
        elif test_:
            test_  = str(root / "data" / test_)
    ...
    torch.save(model, save_dir / "init.pt")
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt["notest"] or final_epoch:  # Calculate mAP
                if epoch >= 0:
                    results, maps, times = test.run_test(
                        opt        = opt,
                        data       = opt["data"],
                        batch_size = batch_size * 2,
                        imgsz      = imgsz_test,
    ...
    if rank in [-1, 0]:
        ...
        flast      = save_dir / f"last{n}.pt"
        fbest      = save_dir / f"best{n}.pt"
        fbest_f1   = save_dir / f"best_f1{n}.pt"
        fbest_ap50 = save_dir / f"best_ap50{n}.pt"
        fbest_ap   = save_dir / f"best_ap{n}.pt"
        for f1, f2 in zip(
            [last,  best,  best_f1,  best_ap,    best_ap,  results_file],
            [flast, fbest, fbest_f1, fbest_ap50, fbest_ap, fresults]
        ):
            if f1.exists():
...
def train(args: dict) -> str:
    ...
    if not mon.Path(opt["model"]).is_yaml_file():
        opt["model"] = str(current_dir / "option" / "model"   / f"{opt["model"]}")
    if not mon.Path(opt["data"]).is_yaml_file():
        opt["data"]  = str(current_dir / "option" / "dataset" / f"{opt["data"]}")
    if not mon.Path(opt["names"]).is_file():
        opt["names"] = str(current_dir / "option" / "dataset" / f"{opt["names"]}")
    if not mon.Path(opt["hyp"]).is_yaml_file():
        opt["hyp"]   = str(current_dir / "option" / "hyp"     / f"{opt["hyp"]}")

    weights    = weights[0] if isinstance(weights, list | tuple) else weights
    weights    = mon.parse_weights_file(root, weights) if weights else None
    pretrained = opt["weights"]
    ...
    if weights and weights.is_weights_file(exist=True):
        ...
        opt["weights"] = pretrained
    else:
        opt["weights"] = None

    opt["root"]     = root
    opt["project"]  = str(save_dir.parent)
    opt["name"]     = str(save_dir.name)
    opt["save_dir"] = save_dir
    opt["epochs"]   = epochs
    opt["imgsz"]    = mon.image_size(imgsz)
    ...
    if opt["resume"]:  # resume an interrupted run
        ...
        with open(mon.Path(ckpt).parent.parent / "opts.yaml") as f:
            opt = argparse.Namespace(**yaml.load(f, Loader=yaml.FullLoader))  # replace
        opt["model"], opt["weights"], opt["resume"] = "", ckpt, True
        logger.info("Resuming training from %s" % ckpt)
    else:
        # opts.hyp = opts.hyp or ('hyp.finetune.yaml' if opts.weights else 'hyp.scratch.yaml')
    ...
    if not opt["evolve"]:
        ...
        run_train(hyp, opt, device, tb_writer, wandb)
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            # Train mutation
            results = run_train(hyp.copy(), opt, device, wandb=wandb)
