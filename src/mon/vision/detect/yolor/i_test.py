...
def run_test(
    ...
    if training:  # called by train.py
        ...
        # Directories
        (save_dir / "image" if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir
    ...
    with open(data, encoding="utf-8") as f:
        ...
        if isinstance(_train, list):
            _train = [str(root / "data" / t) for t in _train]
        elif _train:
            _train = str(root / "data" / _train)
        if isinstance(_val, list):
            _val   = [str(root / "data" / t) for t in _val]
        elif _val:
            _val   = str(root / "data" / _val)
        if isinstance(_test, list):
            _test  = [str(root / "data" / t) for t in _test]
        elif _test:
            _test  = str(root / "data" / _test)
        data["train"] = _train
        data["val"]   = _val
        data["test"]  = _test
