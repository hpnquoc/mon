...
def run_predict(opt: dict, save_img: bool = False):
    source   = opt["source"]
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            save_path  = str(save_dir / "image" / f"{mon.Path(p).stem}.jpg")
            txt_path   = str(save_dir / "label" / mon.Path(p).stem) + ("_%g" % dataset.frame if dataset.mode == "video" else "")
