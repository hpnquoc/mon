...
class YOLOv8(base.Detector1):
    ...
    def preprocess(self, images: np.ndarray) -> torch.Tensor:
        ...
        """
        input  = images.copy()
        ratio  = max(self.image_size) / max(datatype.get_image_size(input=input))
        stride = self.model.stride
        ...
        input = datatype.to_image_tensor(
            image= input,
            keepdim   = False,
            normalize = True,
            device    = self.device
    ...
    def postprocess(
        ...
        *args, **kwargs
        ...
        h0, w0 = datatype.get_image_size(input=images)
        h1, w1 = datatype.get_image_size(input=input)
