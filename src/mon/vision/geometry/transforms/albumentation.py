...
class CropPatch(DualTransform):
    ...
    References:
        - https://github.com/ZhendongWang6/Uformer/blob/main/dataset/dataset_denoise.py
    """

    _targets = (Targets.IMAGE, Targets.MASK)

    class InitSchema(BaseModel):
        patch_size: int   = Field(ge=1, description="Patch size.")
        p         : float = 1.0
    ...
    def apply(self, img: np.ndarray, r: int, c: int, *args: Any, **params: Any) -> np.ndarray:
        return img[r:r + self.patch_size, c:c + self.patch_size, :]

    def apply_to_mask(self, img: np.ndarray, r: int, c: int, *args: Any, **params: Any) -> np.ndarray:
        return img[r:r + self.patch_size, c:c + self.patch_size, :]
...
class ResizeMultipleOf(DualTransform):
    ...
    def apply(self, img: np.ndarray, interpolation: int, *args: Any, **params: Any) -> np.ndarray:
    ...
    def apply_to_bboxes(self, bbox: np.ndarray, *args: Any, **params: Any) -> np.ndarray:
    ...
    def apply_to_keypoints(self, keypoints: np.ndarray, *args: Any, **params: Any) -> np.ndarray:
        ...
        return F.keypoint_scale(keypoints, scale_x, scale_y)
...
class NormalizeImageMeanStd(DualTransform):
    ...
    def apply_to_mask(self, img: np.ndarray, *args: Any, **params: Any) -> np.ndarray:
        return (img - self.mean) / self.std
