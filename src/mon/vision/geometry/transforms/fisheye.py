...
"""Implements fisheye transformation functions.
...
__all__ = [
    "FisheyeTransform",
    "fisheye_transform_bbox",
    "fisheye_transform_image",
    "fisheye_transform_image_and_bbox",
]
...
def fisheye_transform_image(image: np.ndarray, distortion: float) -> np.ndarray:
...
def fisheye_transform_bbox(
    ...
    for b in bbox:
        ...
        bbox_x_u = np.zeros_like(bbox_x)
        bbox_y_u = np.zeros_like(bbox_y)
        for i in range(4):
            bbox_x[i] = (2 * bbox_x[i] - w0) / w0
            bbox_y[i] = (2 * bbox_y[i] - h0) / h0
            dr[i]     = np.sqrt(bbox_x[i] ** 2 + bbox_y[i] ** 2)
            bbox_x_u[i], bbox_y_u[i] = reverse_fisheye_xy_n(bbox_x[i], bbox_y[i], dr[i], distortion)
            bbox_x_u[i] = int(((bbox_x_u[i] + 1) * w0) / 2)
            bbox_y_u[i] = int(((bbox_y_u[i] + 1) * h0) / 2)

        x1_new = int(min(bbox_x_u)) - left_margin
        y1_new = int(min(bbox_y_u)) - top_margin
        x2_new = int(max(bbox_x_u)) - left_margin
        y2_new = int(max(bbox_y_u)) - top_margin

        # Convert back to normalized YOLO format
        cx_n_new = (x1_new + x2_new) / 2 / w1
        cy_n_new = (y1_new + y2_new) / 2 / h1
        w_n_new  = (x2_new - x1_new) / w1
        h_n_new  = (y2_new - y1_new) / h1

        # Preserve additional fields (e.g., class ID, confidence) if M>4
        new_bbox.append(np.concatenate(([cx_n_new, cy_n_new, w_n_new, h_n_new], b[4:])))

    return np.array(new_bbox, dtype=np.float32)


def fisheye_transform_image_and_bbox(
    image     : np.ndarray,
    bbox      : np.ndarray,
    distortion: float
    ...
    Args:
        ...
        distortion: Distortion factor.
    """
    transformed_image  = fisheye_transform_image(image, distortion)
    w0, h0 = types.image_size(image)
    w1, h1 = types.image_size(transformed_image)
    transformed_bboxes = fisheye_transform_bbox(bbox, (w0, h0), (w1, h1), distortion)
...
class FisheyeTransform(DualTransform):
    ...
    def __init__(self, distortion: float = 1.0, p: float = 1):
        super().__init__(p=p)
    ...
    def apply_to_mask(
        ...
        *args: Any, **params: Any
        ...
        return fisheye_transform_image(img, self.distortion)
    ...
    def apply_to_bboxes(
        ...
        *args: Any, **params: Any
        ...
        return fisheye_transform_bbox(bboxes, old_size, new_size, self.distortion)

    def get_params_dependent_on_data(self, params: dict[str, Any], data: dict[str, Any]) -> dict[str, Any]:
        """Returns parameters dependent on input."""
        image         = data["image"]
        fisheye_image = fisheye_transform_image(image, self.distortion)
