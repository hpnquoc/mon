...
class Instance:
    ...
    Attributes:
        ...
        classlabel: A `mon.Classlabel` object. Default: ``None``.
    ...
    def draw(
        ...
        """Draw the current object on the `image`."""
...
class MovingObject(list[Instance], Object):
    ...
    def traveled_distance_between(self, start: int = -1, end: int = -2) -> float:
        """The traveled distance of the object between `start` and
        `end` frames.
        """
        step = abs(end - start)
    ...
    def draw(
        ...
        """Draw the current object and its trajectory on the `image`."""
        if self.moi_id:
