...
class SegmentationModel(model.VisionModel, ABC):
    ...
    def log_images(self, epoch: int, step: int, data: dict, extension: str = ".jpg"):
        ...
        tar_semantic  = list(datatype.convert_image_to_array(tar_semantic, denormalize=True)) if tar_semantic is not None else None
        pred_semantic = list(datatype.convert_image_to_array(pred_semantic, denormalize=True))
