...
class Downsampler(nn.Module):
    ...
    def __init__(
        ...
        # note that `kernel width` will be different to actual size for phase
        # = 1/2
    ...
    if kernel_type == 'box':
        ...
        for i in range(1, kernel.shape[0] + 1):
            for j in range(1, kernel.shape[1] + 1):
                ...
                val = 1
                if di != 0:
                    val = val * support * np.sin(np.pi * di) * np.sin(
                        np.pi * di / support
                        )
                    val = val / (np.pi * np.pi * di * di)
                
                if dj != 0:
                    val = val * support * np.sin(np.pi * dj) * np.sin(
                        np.pi * dj / support
                        )
    ...
    for i in range(1, downsampling_number + 1):
        im = np.clip(
            imresize(
                ...
                scale_factor=(1 - (downsampling_factor * downsampling_number))
            ).transpose(2, 0, 1),
            0,
            1
        )
