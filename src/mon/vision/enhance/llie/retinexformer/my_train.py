...
def create_train_val_dataloader(opt, logger):
    ...
    for phase, dataset_opt in opt["datasets"].items():
        if phase == "train":
            ...
            train_sampler = EnlargedSampler(train_set, opt["world_size"], opt["rank"], dataset_enlarge_ratio)  # 缺少关键字 world_size 和 rank，train_sampler是做什么？从get_dist_info得到
...
def train(args: argparse.Namespace):
    ...
    # device   = args.device
    # device   = device if isinstance(device, (list, tuple)) else [device]
    # gpu_list = ",".join(str(x) for x in device)
    ...
    if resume_state is None:
        ...
        if opt["logger"].get("use_tb_logger") and "debug" not in opt["name"] and opt["rank"] == 0:
            ...
            mkdir_and_rename2(osp.join(opt["path"]["tb_logger"], opt["name"]), opt["rename_flag"])
