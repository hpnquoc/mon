    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        for i in range(self.num_scales):
            ...
            self.value_nets.append(nn.INRPatchEncoder(window_size_, mid_channels_, down_size_, hidden_layers_, omega_0_, first_bias_scale_, nonlinear_, weight_decay[1]))
            self.coords_nets.append(nn.INRCoordinatesEncoder(mid_channels_, down_size_, hidden_layers_, omega_0_, first_bias_scale_, nonlinear_, weight_decay[0]))
            self.output_nets.append(nn.INRDecoder(hidden_channels_, self.out_channels, out_layers_, omega_0_, nonlinear_, weight_decay[2]))
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        for i in range(self.num_scales):
            ...
            self.value_nets.append(nn.INRPatchEncoder(window_size_, mid_channels_, down_size_, hidden_layers_, omega_0_, first_bias_scale_, nonlinear_, weight_decay[1]))
            self.depth_nets.append(nn.INRPatchEncoder(window_size_, mid_channels_, down_size_, hidden_layers_, omega_0_, first_bias_scale_, nonlinear_, weight_decay[1]))
            self.edge_nets.append(nn.INRPatchEncoder(window_size_, mid_channels_, down_size_, hidden_layers_, omega_0_, first_bias_scale_, nonlinear_, weight_decay[1]))
            self.coords_nets.append(nn.INRCoordinatesEncoder(mid_channels_, down_size_, hidden_layers_, omega_0_, first_bias_scale_, nonlinear_, weight_decay[0]))
            self.output_nets.append(nn.INRDecoder(hidden_channels_, self.out_channels, out_layers_, omega_0_, nonlinear_, weight_decay[2]))
        self.dba = nn.BoundaryAwarePrior(eps=dba_eps, normalized=False)
        self.w_0 = nn.Parameter(torch.Tensor([0.5]))
