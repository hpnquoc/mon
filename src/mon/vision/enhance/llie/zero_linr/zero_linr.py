    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        for i in range(self.num_scales):
            ...
            self.value_nets.append(nn.INRPatchEncoder(window_size_, mid_channels, down_size, hidden_layers, omega_0, first_bias_scale, nonlinear, weight_decay[1]))
        self.depth_net  = nn.INRPatchEncoder(window_size[-1], mid_channels, down_size, hidden_layers, omega_0, first_bias_scale, nonlinear, weight_decay[1])
...
class MLP_HSV(MLP):
    ...
    def forward(self, image: torch.Tensor, depth: torch.Tensor = None) -> torch.Tensor:
        ...
        enhanced    = dtype.hsv_to_rgb(enhanced.clone())
        # enhanced    = enhanced / torch.max(enhanced)
        # Return
        return {
            "image"       : image,
