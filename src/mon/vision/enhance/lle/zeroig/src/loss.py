...
from .utils import calculate_local_variance, LocalMean, pair_downsampler
...
class LossFunction(nn.Module):
    ...
    def forward(
        self,
        input, L_pred1, L_pred2, L2, s2, s21, s22, H2, H11, H12, H13, s13, H14, s14, H3, s3,
...
class TextureDifference(nn.Module):
    ...
    def forward(self, image1, image2):
        ...
        binary_diff = torch.where(diff > self.threshold, torch.tensor(1.0, device=diff.device), torch.tensor(0.0, device=diff.device))
        return binary_diff
