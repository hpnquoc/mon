...
def main():
    ...
    for i, img_path in enumerate(img_list):
        ...
        if img_type == "JPEG":
            ...
            im_Gen_lpips = lpips.im2tensor(lpips.load_image(os.path.join(args.generated_images_path, base_name + suffix + '.jpg')))
        elif img_type == "PNG":
            im_Gen       = cv2.imread(os.path.join(args.generated_images_path, base_name + suffix + '.png')) / 255.
            im_Gen_lpips = lpips.im2tensor(lpips.load_image(os.path.join(args.generated_images_path, base_name + suffix + '.png')))
        else:
            print("不支持的图像类型")
