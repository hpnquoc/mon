...
class BSConv2dS(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input: Input tensor as a ``torch.Tensor`` with shape [B, C_in, H, W].

        Returns:
            Output tensor as a ``torch.Tensor`` with shape [B, C_out, H_out, W_out].
        """
        y = self.pw1(input)
        if self.bn1:
            y = self.bn1(y)
    ...
    def regularization_loss(self) -> torch.Tensor:
        ...
        Returns:
            Frobenius norm of weight correlation matrix deviation as a ``torch.Tensor``.
        """
