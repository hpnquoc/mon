...
def create_coords(down_size: int) -> torch.Tensor:
    ...
    Returns:
        Tensor of shape [down_size, down_size, 2] with normalized coords as a ``torch.Tensor``.
    """
...
def create_patches(image: torch.Tensor, kernel_size: int = 1) -> torch.Tensor:
    ...
    Args:
        image: Tensor of shape [C, H, W] or [B, C, H, W] as a ``torch.Tensor``.
        kernel_size: Size of square patches as ``int``. Default is ``1``.

    Returns:
        Tensor with patches in channels as a ``torch.Tensor``,
...
def interpolate_image(image: torch.Tensor, down_size: int) -> torch.Tensor:
    ...
    Returns:
        Resized tensor as a ``torch.Tensor`` of shape
...
def ff_embedding(p: torch.Tensor, B: torch.Tensor = None) -> torch.Tensor:
    ...
    Args:
        p: Input tensor to embed as a ``torch.Tensor``.
        B: Projection matrix as a ``torch.Tensor`` or ``None``. Default is ``None``.

    Returns:
        Embedded tensor as a ``torch.Tensor`` with sine and cosine features.
    """
    if B is None:
        return p
