...
class AdaptiveBatchNorm2d(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input: Tensor to normalize as a ``torch.Tensor``.

        Returns:
            Normalized tensor as a ``torch.Tensor``.
        """
...
class LearnableInstanceNorm2d(nn.InstanceNorm2d):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input: Tensor of shape [B, C, H, W] to normalize as a ``torch.Tensor``.

        Returns:
            Partially normalized tensor as a ``torch.Tensor``.
        """
...
class HalfInstanceNorm2d(nn.InstanceNorm2d):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input: Tensor of shape [B, C, H, W] or [L, C, T] as a ``torch.Tensor``.

        Returns:
            Tensor with first half normalized as a ``torch.Tensor``.
...
class LayerNorm2d(nn.LayerNorm):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input: Tensor of shape [B, C, H, W] as a ``torch.Tensor``.

        Returns:
            Normalized tensor of shape [B, C, H, W] as a ``torch.Tensor``.
        """
        return F.layer_norm(
