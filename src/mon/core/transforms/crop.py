...
def split_image_and_hbbs(image: np.ndarray, bbox: np.ndarray, n: int = 2) -> tuple[list[np.ndarray], list[np.ndarray]]:
    """Split an image with the accompanying HBBs into ``n`` equal parts
...
def center_crop_image_and_hbbs(image: np.ndarray, bbox: np.ndarray, imgsz: int) -> tuple[np.ndarray, np.ndarray]:
    """Center crop an image with the accompanying HBBs.
    ...
    bbox = B.hbb.convert(bbox, fmt=BBoxFormat.CXCYWHN2XYXY, imgsz=(h0, w0))
    ...
    adjusted_bbox = B.hbb.convert(adjusted_bbox, fmt=BBoxFormat.XYXY2CXCYWHN, imgsz=(h1, w1))
