...
class VideoWriter(ABC):
    ...
    Args:
        ...
        imgsz: Output video size as a ``tuple`` of :math:`(H, W)`. Default is ``(480, 640)``.
        frame_rate: Frame rate of output video. Default is ``30``.
    ...
    def write(self, frame: np.ndarray, path: Path = None):
        ...
        Args:
            frame: A video frame as a ``numpy.ndarray`` of shape :math:`(H, W, C)`.
            path: Optional path to save ``frame`` as image. Default is ``None``.
        """
        pass
...
class VideoWriterFFmpeg(VideoWriter):
    ...
    Args:
        ...
        imgsz: Output video size as a ``tuple`` of :math:`(H, W)`. Default is ``(480, 640)``.
        frame_rate: Frame rate of output video. Default is ``30``.
        pix_fmt: Video codec. Default is ``"yuv420p"``.
        verbose: Enable verbosity if ``True``. Default is ``False``.
    """
