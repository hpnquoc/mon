...
def color_transfer(source: np.ndarray, target: np.ndarray) -> np.ndarray:
    ...
    Args:
        source: Source image as a ``numpy.ndarray`` in RGB format.
        target: Target image as a ``numpy.ndarray`` in RGB format.

    Returns:
        Color transferred image as a ``numpy.ndarray`` in RGB format.
    """
