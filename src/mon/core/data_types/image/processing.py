...
def split(image: Union[torch.Tensor, np.ndarray], n: int = 2) -> list[np.ndarray]:
    ...
    Args:
        image: Image as a ``numpy.ndarray`` [H, W, C].
...
def to_channel_first(image: Union[torch.Tensor, np.ndarray]) -> Union[torch.Tensor, np.ndarray]:
    ...
    Returns:
        Channel-first image as a ``torch.Tensor`` [C, H, W] or [B, C, H, W], or
...
def to_channel_last(image: Union[torch.Tensor, np.ndarray]) -> Union[torch.Tensor, np.ndarray]:
    ...
    Returns:
        Channel-last image as a ``torch.Tensor`` [H, W, C] or [B, H, W, C], or
...
def to_array(image: torch.Tensor) -> np.ndarray:
    ...
    Args:
        image: RGB image as a ``torch.Tensor`` [B, C, H, W].
    
    Returns:
        Image as a ``numpy.ndarray`` in [H, W, C].
...
def to_tensor(image: np.ndarray, normalize: bool = False) -> torch.Tensor:
    ...
    Args:
        image: RGB image as a ``numpy.ndarray`` [H, W, C].
        normalize: Normalize to [0.0, 1.0] if ``True``. Default is ``False``.

    Returns:
        Image as a ``torch.Tensor`` in [B, C, H, W] format.
