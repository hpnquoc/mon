...
def load_image(path: Path, flags: int = cv2.IMREAD_COLOR) -> np.ndarray:
    ...
    Returns:
        An RGB or grayscale image as a ``numpy.ndarray`` of shape ``(H,W,C)``.
    """
    path = Path(path)
...
def read_image_shape(path: Path) -> tuple[int, int, int]:
    ...
    Returns:
        A tuple of ``(height, width, channels)``.
...
def save_image(image: Union[torch.Tensor, np.ndarray], path: Path):
    ...
    Args:
        image: Image as a ``torch.Tensor`` (e.g., shape ``(B,C,H,W)`` in range ``[0.0,1.0]``)
