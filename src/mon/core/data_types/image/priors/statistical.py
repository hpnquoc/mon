...
def blur_spot_prior(image: np.ndarray, threshold: int = 250) -> bool:
    ...
    Args:
        image: Input image as a ``numpy.ndarray`` in BGR format with shape [H, W, 3].
...
def bright_channel_prior(
    ...
    Args:
        image: RGB image as a ``torch.Tensor`` [B, C, H, W] or ``numpy.ndarray`` [H, W, C].
        kernel_size: Window size as int or tuple.

    Returns:
        Bright channel prior as a ``torch.Tensor`` or ``numpy.ndarray``.
    """
...
def dark_channel_prior(
    ...
    Returns:
        Dark channel prior as a ``torch.Tensor`` or ``numpy.ndarray``.
    """
    kernel_size = to_2tuple(kernel_size)
    if isinstance(image, torch.Tensor):
