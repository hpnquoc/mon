...
from mon.core.data_types.image import utils


def brightness_attention_map(
    image        : Union[torch.Tensor, np.ndarray],
    gamma        : float = 2.5,
    denoise_ksize: int   = None,
) -> torch.Tensor:
    ...
    Returns:
        Brightness enhancement map as a ``torch.Tensor`` or ``numpy.ndarray`` matching
        input type.
    """
...
class BrightnessAttentionMap(nn.Module):
    ...
    def __init__(self, gamma: float = 2.5, denoise_ksize: int = None):
        super().__init__()
        self.gamma         = gamma
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            image: Input image tensor of shape [B, C, H, W] as a ``torch.Tensor``  with
                data in range [0.0, 1.0].

        Returns:
            Brightness attention map tensor as a ``torch.Tensor`` with shape [B, C, H, W].
        """
