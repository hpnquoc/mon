...
def atmospheric_point_spread_function(
    ...
    Args:
        image: An RGB image as a ``torch.Tensor`` [B, C, H, W] in range [0.0, 1.0] or
    ...
    Returns:
        APSF as a ``torch.Tensor`` or ``numpy.ndarray``.
...
def atmospheric_prior(image: np.ndarray, kernel_size: int = 15, p: float = 0.0001) -> np.ndarray:
    """Get the atmospheric light in an RGB image.

    Args:
        image: RGB image as a ``numpy.ndarray`` in [H, W, C] format with data in range [0, 255].
    ...
    Returns:
        3-element array of atmospheric light as a ``numpy.ndarray`` in range [0, 255] for each channel.
    """
    ...
    dark       = dark_channel_prior_paper(image=image, kernel_size=kernel_size)
    m, n       = dark.shape
