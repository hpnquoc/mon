...
def center(image: Union[torch.Tensor, np.ndarray]) -> Union[torch.Tensor, np.ndarray]:
    ...
    Args:
        image: Image as a ``torch.Tensor`` or ``numpy.ndarray``.
    
    Returns:
        Center coordinates as a ``torch.Tensor`` or ``numpy.ndarray`` with shape [2].
    """
    h, w    = imgsz(image)
    center_ = [h / 2, w / 2]
...
def center4(image: Union[torch.Tensor, np.ndarray]) -> Union[torch.Tensor, np.ndarray]:
    ...
    Returns:
        Center coordinates as a ``torch.Tensor`` or ``numpy.ndarray`` with shape [4].
    """
    h, w    = imgsz(image)
...
def shape(image: Union[torch.Tensor, np.ndarray]) -> list[int]:
    """Returns height, width, and channels of an image.

    Args:
        image: RGB image as a ``torch.Tensor`` with shape [B, C, H, W] or
...
def imgsz(input: Union[torch.Tensor, np.ndarray, int, list, tuple], divisor: int = None) -> tuple[int, int]:
    """Returns height and width of an image in [H, W] format.

    Args:
        input: RGB image, tensor, array, size, or path as a ``torch.Tensor``,
...
def is_channel_first(image: Union[torch.Tensor, np.ndarray]) -> bool:
    ...
    Args:
        image: Image as a ``torch.Tensor`` or ``numpy.ndarray`` in [C, H, W] or
            [B, C, H, W] format.
...
def is_channel_last(image: Union[torch.Tensor, np.ndarray]) -> bool:
    ...
    Args:
        image: Image as a ``torch.Tensor`` or ``numpy.ndarray`` in [H, W, C] or
            [B, H, W, C] format.
