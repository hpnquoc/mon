...
class HBBs(BaseTensorOrArray):
    ...
    Args:
        data: Input data as a ``torch.Tensor`` or ``numpy.ndarray``. If given,
            it must be of shape :math:`(N, 4+)` in ``XYXY`` format. Default is ``None``.
        ...
        fmt: One of the bounding box format from ``BBoxFormat``. Default is ``BBoxFormat.XYXY``.
    ...
    def __init__(
        ...
        if data is not None and orig_shape is not None:
            from mon.core.data_types.bbox.hbb import utils
