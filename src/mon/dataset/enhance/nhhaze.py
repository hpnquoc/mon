...
class NHHaze(vision.VisionDataset):
    """Loads NH-Haze dataset from ``root`` dir.
    ...
    def __init__(self, root: core.Path = DATA_DIR / "enhance", *args, **kwargs):
        ...
        root = root / "nhhaze" if root.name != "nhhaze" else root
        if not root.is_dir():
...
class NHHazeDataModule(core.DataModule):
    ...
    def setup(self, stage: Literal["train", "test", "predict", None] = None):
        ...
        if stage in [None, "train"]:
            self.train = NHHaze(split=core.Split.TRAIN, **self.dataset_kwargs)
            self.val   = NHHaze(split=core.Split.VAL,   **self.dataset_kwargs)
        if stage in [None, "test"]:
            self.test  = NHHaze(split=core.Split.TEST,  **self.dataset_kwargs)
