...
class SIRENLayer(nn.Module):
    ...
    Args:
        ...
        w0: The frequency of the sine activation function. Defaults: ``1.0``.
    ...
    def __init__(
        ...
        self.linear       = nn.Linear(self.in_channels, self.out_channels, bias=self.use_bias)
        if self.is_last:
            self.activation = act.Sigmoid() if activation is None else activation
        else:
            self.activation = act.Sine(self.w0) if activation is None else activation
    ...
    def init_weights(self):
        if self.is_first:
            ...
            w_std = math.sqrt(self.C / self.in_channels) / self.w0
        with torch.no_grad():
...
class SIREN(nn.Module):
    ...
    Args:
        ...
        n_hidden_layers: The number of layers.
        w0: The frequency of the sine activation function. Defaults: ``1.0``.
        w0_first: The initial frequency of the sine activation function.
            Defaults: ``30.0``.
        use_bias: Whether to use bias. Defaults: ``True``.
        dropout: The dropout rate. Defaults: ``0.0``.
    ...
    def __init__(
        ...
        self.layers.append(SIRENLayer(in_channels, hidden_channels, w0=w0_first, is_first=True, use_bias=use_bias, dropout=dropout))
        
        for idx in range(n_hidden_layers):
            is_first          = idx == 0
            layer_w0          = w0_first    if is_first else w0
            layer_in_channels = in_channels if is_first else hidden_channels
            layer = SIRENLayer(
        ...
        if last_linear:
            last_layer = nn.Linear(hidden_channels, out_channels)
            with torch.no_grad():
                last_layer.weight.uniform_(-np.sqrt(6 / hidden_channels) / w0, np.sqrt(6 / hidden_channels) / w0)
            self.last_layer = last_layer
        else:
            last_activation = nn.Identity() if last_activation is None else last_activation
    ...
    def forward(self, input: torch.Tensor, mods: torch.Tensor = None):
        mods = self.cast_tuple(mods, self.n_hidden_layers)
        x    = input
