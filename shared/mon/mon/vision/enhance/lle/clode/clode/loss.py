...
class L_spa(nn.Module):
    ...
    def forward(self, org: torch.Tensor, enhance: torch.Tensor) -> torch.Tensor:
        ...
        weight_diff = torch.max(
            torch.FloatTensor([1]).to(deivce) + 10000 * torch.min(org_pool - torch.FloatTensor([0.3]).to(deivce),
                                                                  torch.FloatTensor([0]).cuda()),
            torch.FloatTensor([0.5]).to(deivce)
        )
...
class GANLoss(nn.Module):
    ...
    def get_target_label(self, input: torch.Tensor, target_is_real: torch.Tensor) -> torch.Tensor:
    ...
    def forward(self, input: torch.Tensor, target_is_real: torch.Tensor):
...
def histeq(im, nbr_bins: int = 256):
