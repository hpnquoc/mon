...
def train(args: dict | box.Box) -> str:
    ...
    # Model
    model = mefode.MEFODE(weights=pretrained)
    ...
    # Loss
    L_tv      = mefode.L_tv().to(device)
    L_spa     = mefode.L_spa().to(device)
    L_col     = mefode.L_col().to(device)
    L_exp_val = mefode.L_exp_value(16, args.loss.L_exp_mean).to(device)
